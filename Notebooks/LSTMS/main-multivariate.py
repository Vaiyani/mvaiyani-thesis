...
def model(lookback, pred_len, num_features):

    model = keras.Sequential()
    model.add(LSTM(100, activation= 'tanh',input_shape=(lookback, num_features)))
...
def model2(lookback, pred_len, num_features):

    model = keras.Sequential()
    model.add(LSTM(50, activation= 'tanh',input_shape=(lookback, num_features)))
    model.add(Dropout(0.2))
    model.add(RepeatVector(pred_len))
    model.add(TimeDistributed(Dense(num_features)))
    model.compile(optimizer=keras.optimizers.Adam(learning_rate=0.00001),loss='mse')
...
def calculate_metrics(test: np.ndarray, predict: np.ndarray, identifier: str) -> float:
    ...
    f = open("result-lstm-multivariate-more-feature-parallel.txt", 'a')
    f.write(identifier + "  \n")
...
if reduce_complexity:
    model = model2(lookback, pred_len, num_features)
else:
    model = model(lookback, pred_len, num_features)


# stop_training_early = keras.callbacks.EarlyStopping()
stop_training_early = keras.callbacks.EarlyStopping(monitor="val_loss", patience=7, restore_best_weights=True)
history = model.fit(x_train, y_train, epochs=50 , verbose=1, shuffle=False, validation_data=(x_val, y_val),
...
calculate_metrics(y_test[:,:-1], y_predict[:,:,-1], identifier)
